...
func TestStoreAndFindLargeValue(t *testing.T) {
	...
	hashKey := sha1.Sum(payload[:])
	key := hashKey[:]
	keyString, err := dht1.Store(key, payload[:])
	assert.NoError(t, err)

	time.Sleep(1 * time.Second)

	value, exists, err := dht2.Get(keyString)
	assert.NoError(t, err)
...
func TestStoreReplication(t *testing.T) {
	...
	dht.Bootstrap()
	key := sha1.Sum([]byte("foo"))
	keyBytes := key[:]
	dht.Store(keyBytes, []byte("foo"))
...
func TestStoreExpiration(t *testing.T) {
	...
	key := sha1.Sum([]byte("foo"))
	keyBytes := key[:]
	k, _ := dht.Store(keyBytes, []byte("foo"))
