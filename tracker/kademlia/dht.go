...
func (dht *DHT) Store(key []byte, data []byte) (id string, err error) {
	expiration := dht.getExpirationTime(key)
	replication := time.Now().Add(dht.options.TReplicate)
	dataToStore := [20]byte(data)
	dht.store.Store(key, dataToStore, replication, expiration, true)
	...
	for {
		...
		case msg := <-dht.networking.getMessage():
			...
			case messageTypeStore:
				...
				dht.addNode(newNode(msg.Sender))
				dataForRequest := [20]byte(data.Data)
				key := dht.store.GetKey(dataForRequest)
				expiration := dht.getExpirationTime(key)
				replication := time.Now().Add(dht.options.TReplicate)
				dht.store.Store(key, dataForRequest, replication, expiration, false)
