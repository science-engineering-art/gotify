...
type Store interface {
	...
	Store(key []byte, data [20]byte, replication time.Time, expiration time.Time, publisher bool) error
...
func (ms *MemoryStore) Init() {
	ms.data = make(map[string][][20]byte)
	ms.mutex = &sync.Mutex{}
...
// GetKey returns the key for data
func (ms *MemoryStore) GetKey(data [20]byte) []byte {
	sha := sha1.Sum(data[:20])
	return sha[:]
}

// Store will store a key/value pair for the local node with the given
// replication and expiration times.
func (ms *MemoryStore) Store(key []byte, data [20]byte, replication time.Time, expiration time.Time, publisher bool) error {
	ms.mutex.Lock()
	defer ms.mutex.Unlock()
	ms.replicateMap[string(key)] = replication
	ms.expireMap[string(key)] = expiration
	ms.data[string(key)] = append(ms.data[string(key)], data)
	return nil
}

// Retrieve will return the local key/value if it exists
func (ms *MemoryStore) Retrieve(key []byte) (returnData []byte, found bool) {
	ms.mutex.Lock()
	defer ms.mutex.Unlock()
	data, found := ms.data[string(key)]
	flatArray := getFlatByteArray(data)
	return flatArray, found
}
...
func getFlatByteArray(array [][20]byte) []byte {
	flatByteArray := []byte{}

	for _, byteArray := range array {
		for _, b := range byteArray {
			flatByteArray = append(flatByteArray, b)
		}
	}
